...
layout (location = 4) flat in uint inCelShaded;
layout (location = 5) in float inAmbient;
layout (location = 6) in vec3 inColor;
layout (location = 7) in vec4 inRim;
...
void main() 
{
    float BandInterval = float(inCelShaded);
    float InvBand = 1.0 / BandInterval;
    float ValueInterval = 1.0 / (BandInterval - 1.0);
    ...
    vec3 normal     = getNormalFromMap(inUV);
    ...
    for(uint i = 1; i < lightCount + 1; ++i) 
    {
        ...
        // specular = clamp(ValueInterval * ceil(specularBand * FScale / BandInterval), 0.0, 1.0);
        ...
        specular     = numerator / max(denominator, 0.001);  
         //specular = clamp(ValueInterval * floor((specularBand * FScale)/ InvBand), 0.0, 1.0);
        ...
        NDotL = clamp(ValueInterval * floor((NDotL * extraWeight)  / InvBand), 0.0, 1.0);
